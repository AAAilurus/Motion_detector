...
def speak_message():
    ...
    engine.say("Alert! Motion has been detected. Video has been saved and emailed.")  # message to speak
...
def send_email_with_attachment(filename):
    ...
    with open(filename, 'rb') as f:
        ...
        msg.add_attachment(file_data, maintype='video', subtype='avi', filename=file_name)  # attach the image
...
while True:  # loop forever
    ...
    if motion_detected:
        ...
        filename = f"motion_{timestamp}.avi"  # image file name
        print(f"[INFO] Motion detected! Video saved as {filename}")
        speak_message()  # call function to speak message
     
       fourcc = cv2.VideoWriter_fourcc(*'XVID')
       out = cv2.VideoWriter(filename, fourcc, 20.0, (500, frame.shape[0]))
      ...
      out.release()
      print(f"[INFO] Video saved as {filename}")
